...
class AppDatabase {
  ...
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      // add line_discount to existing sale_items if missing
      await db.execute('ALTER TABLE sale_items ADD COLUMN line_discount REAL NOT NULL DEFAULT 0;');
      // ensure subtotal exists
      // (if your previous version omitted subtotal, you can re-create or ignore since it's calculated on insert)
      try {
        await db.execute('ALTER TABLE sale_items ADD COLUMN subtotal REAL NOT NULL DEFAULT 0;');
      } catch (_) {}
      await db.execute('CREATE INDEX IF NOT EXISTS idx_sales_created_at ON sales(created_at);');
    }
  }
